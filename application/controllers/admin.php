...
class Admin extends CI_Controller {
	...
	public function general_users()
	{
 	...
 	$query= $this->db->query('Drop table if exists pas_view');

 	$sql="CREATE Table pas_view  select user_id, '". $Ausdate ."' as dum ,first_name, last_name, S1.email, garments, active, group_name, infusionsoft_id, creation_date, last_login,login_per_Mnth, Days_Since_Last_Login, uacc_group_fk from ( select pas_user_info.user_id, first_name, last_name, pas_user_accounts.uacc_email AS email, pas_garment_info.garments, IF (uacc_active = 1, 'fa fa-check-circle', 'glyphicon glyphicon-ban-circle') AS active, pas_user_groups.ugrp_desc AS group_name, infusionsoft_id, pas_user_accounts.uacc_date_added AS creation_date, pas_user_accounts.uacc_date_last_login AS last_login, datediff('".   $Ausdate  ."', pas_user_accounts.uacc_date_last_login) as Days_Since_Last_Login, uacc_group_fk from pas_user_info join pas_user_accounts on pas_user_info.user_id = pas_user_accounts.uacc_id join pas_user_groups on pas_user_groups.ugrp_id = pas_user_accounts.uacc_group_fk join pas_user_infusionsoft  on pas_user_info.user_id = pas_user_infusionsoft.user_id left join  (SELECT import_user_id, COUNT(garment_id) AS garments FROM pas_garment GROUP BY import_user_id) as pas_garment_info on pas_user_info.user_id = pas_garment_info.import_user_id ) S1 left join ( select email, count(*) login_per_Mnth from pas_user_logins where Month(Login) = Month ( current_timestamp()) group by email) as S2  on S1.email=S2.email ";
	...
	public function question_comments(){
		$this->login_check();
		$this->datatables->select("pas_assessment_comment.garment_id, CONCAT(first_name, ' ' , last_name) AS owner, pas_user_info.user_id, pas_garment.name, pas_assessment_comment.date_created, IF (email_sent = 1, 'fa fa-check-circle', 'glyphicon glyphicon-ban-circle') AS email_sent, email_date", FALSE)->from('assessment_comment')->join('garment', 'pas_garment.garment_id = pas_assessment_comment.garment_id', 'left')->join('user_info', 'user_info.user_id = garment.import_user_id');
		$this->datatables->edit_column('owner', '$1 (<a href="/admin/user/view/$2.html">User</a>, <a href="/admin/garment/user/$2.html">Garment</a>)', 'owner, user_id,');
